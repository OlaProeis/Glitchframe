...
* Sample IFNet at ``s_j = inset + (1 - 2*inset) * (j + 0.5)/n`` for ``j ∈ [0, n)`` where ``n = 2**N``. The ``inset`` is **v7.2 capped** by a target boundary-velocity ratio (default **5×** body velocity, override via ``GLITCHFRAME_RIFE_BOUNDARY_VELOCITY_RATIO``) — see the next section for the derivation. At ``rife_exp ≤ 4`` (``n ≤ 16``) the cap is a no-op and ``inset`` stays at the v7.1 default of **0.12**; at higher ``rife_exp`` the cap shrinks it (e.g. ``0.0362`` at ``N = 6``, ``0.0096`` at ``N = 8``). An explicit ``GLITCHFRAME_RIFE_IFNET_INSET`` value bypasses the cap and is honoured byte-exactly (clamped to ``[0, 0.45]``) so byte-exact reproduction against older bakes / manual experimentation continues to work. The closest-to-boundary samples sit at ``s ≈ inset`` and ``s ≈ 1 - inset`` instead of the legacy ``≈ 0.5/n`` / ``≈ 1 - 0.5/n``, so every dense sample carries visible flow displacement and is visually distinct from both bracketing keyframes.
...
The residual IFNet-timestep velocity cost on each side of every internal keyframe is ``inset / (T_seg/(2n)) = 2 · n · inset / T_seg`` per second over the ``T_seg/(2n)`` boundary cell. The **ratio** to the body's own IFNet-timestep velocity ``(1 - 2 · inset) / T_seg`` simplifies to

```
ratio = 2 · n · inset / (1 - 2 · inset)
```

(``T_seg`` cancels). At ``inset = 0.12``, ``n = 16`` that's ~5 × the body velocity — the v7 / v7.1 design target, perceived as a brief speed-up *into* every keyframe rather than a discrete jump. But the ratio scales **linearly in ``n``**: with the *same* constant ``inset = 0.12`` it becomes ~80 × the body velocity at ``rife_exp = 8`` (``n = 256``), which translates to ~25 % IFNet-timestep change per video frame at 30 fps — a clear pre-anchor "skip" *and* (symmetrically, because the post-anchor first body of the next segment sits ``inset = 0.12`` along the next flow) a visible post-anchor cross-pair residual on every keyframe boundary at heavy ``rife_exp``. (This was the dominant artifact reported under v7.1 at ``rife_exp = 8`` — the texture blip was gone, but the velocity spike scales with ``n`` and the v7.1 reference inset was calibrated only at ``n = 16``.)

**v7.2 (schema v9): boundary-velocity cap.** v7.2 caps the inset so the ratio stays bounded for every ``rife_exp``:

```
inset_cap        = ratio_target / (2 · n + 2 · ratio_target)
inset_effective  = min(DEFAULT_IFNET_TIMESTEP_INSET, inset_cap)
```

with ``ratio_target = 5`` by default (override via ``GLITCHFRAME_RIFE_BOUNDARY_VELOCITY_RATIO``, clamped to ``[0, 100]``). At ``rife_exp ≤ 4`` (``n ≤ 16``) the cap evaluates ≥ 0.119 — at or just below the v7.1 default 0.12 — so the ``min`` returns either 0.12 (low ``rife_exp``) or 0.119 (at ``n = 16`` exactly), within the user-perceptible motion threshold of the v7.1 reference. At higher ``rife_exp`` the cap kicks in (e.g. 0.0362 at ``n = 64``, 0.0096 at ``n = 256``), which has two beneficial effects: (a) the pre-anchor IFNet-timestep velocity spike stays at ``ratio_target ×`` body velocity regardless of ``n``, eliminating the visible "skip" right before every keyframe; and (b) the boundary body samples are tightened to ``s ≈ cap`` / ``s ≈ 1 − cap`` (e.g. ``s ≈ 0.011`` / ``s ≈ 0.989`` at ``n = 256``), which **also** minimises the post-anchor cross-pair residual because the first body of the next segment now sits at a flow displacement of only ~1 % toward ``kf_{i+2}`` instead of the v7.1 ~12 %. An explicit ``GLITCHFRAME_RIFE_IFNET_INSET`` value bypasses the cap; ``GLITCHFRAME_RIFE_IFNET_INSET = 0`` still collapses the spike to ~0 (perfect velocity continuity) at the cost of letting the per-pair body samples near every keyframe become visually near-identical to the keyframe itself (the original "stops on the still" symptom at low ``rife_exp``; at high ``rife_exp`` the near-identical wall-clock window shrinks to ``T_seg/n`` and is too brief to perceive).

Cost: **+1 IFNet inference per internal keyframe** vs. v7 (``total_segs − 1`` extra forward passes per bake; sub-second on a 3090 for typical 20–40-keyframe songs). Still well below v6's ``N − 2`` cross-pair bridge calls per boundary. v7.2 does not change the cost relative to v7.1 — only the IFNet timesteps at which the body / bookend samples are evaluated change with ``rife_exp``.
...
**Spacing:** within a segment the wall-clock spacing of body samples is uniform ``T_seg / n`` (e.g. with ``T_seg = 8 s`` and ``N = 4`` that's ``0.5 s``). Across every internal keyframe boundary the spacing is halved to ``T_seg / (2n)`` per side because the v7.1 IFNet-rendered anchor sits exactly on ``t_kf``. The only other spacing irregularity is the short ``T_seg/(2n)`` gap between the velocity-matched bookend and its neighbouring body sample, where IFNet velocity now matches the body — visually smooth. Spacing is **independent of the v7.2 boundary-velocity cap** — only the IFNet timesteps at which the boundary body samples / the bookends / the (unchanged) anchor are evaluated shrink with ``rife_exp``; the wall-clock grid is unaffected.

**Frame count formula:** ``frame_count = total_segs * n_steps + (total_segs - 1) + 2`` — body samples + internal-keyframe anchors + two velocity-matched bookends. For ``N = 31`` keyframes (30 segments) at ``rife_exp = 4`` (``n_steps = 16``): ``30 * 16 + 29 + 2 = 511`` frames (identical count to v6 / v7 / v7.1 / v7.2; only the *content* of the ``total_segs − 1`` boundary slots **and** the IFNet timesteps at which body / bookend samples are evaluated change between schema versions — cross-pair bridge in v6, raw SDXL still in v7, IFNet endpoint render in v7.1, and v7.2 additionally caps the body / bookend inset by the boundary-velocity ratio at high ``rife_exp``).

**Cache invalidation:** ``RIFE_MANIFEST_SCHEMA_VERSION`` is **9** (= v7.2). v1 → centered (v2) introduced no exact internal stills; v2 → v3 added ``keyframes_content_hash`` (SHA-256 over keyframe RGB payloads in order) so **replacing or editing keyframe PNGs** invalidates the RIFE cache even when text prompts and layout are unchanged; v3 → **v4** introduced inset-warped centered sampling; v4 → **v5** replaced the exact-still bookends with velocity-matched IFNet bookends (eliminating the skip at song open / close); v5 → **v6** raised the body inset default ``0.12 → 0.25`` and inserted a cross-pair IFNet bridge at every internal keyframe; v6 → **v7** reverted the inset to ``0.12`` and replaced the cross-pair bridge with the exact SDXL keyframe still at every internal boundary (eliminating the cross-pair pixel-blend region that caused the v6 "skipping entire frames" / "clear move of the object" artifact); v7 → **v8 (v7.1)** replaced the raw SDXL still anchor with ``IFNet(kf_i, kf_{i+1}, s = 1.0)`` after we discovered Practical-RIFE's IFNet has no identity branch at the endpoints — the raw VAE still and the IFNet render differ by a per-network texture signature, and inserting the VAE still as an anchor introduced a 1–2-video-frame texture "blip" at every internal boundary; v8 → **v9 (v7.2)** caps the body / bookend inset by a boundary-velocity ratio target (default 5×) so the pre-anchor IFNet-timestep velocity spike stays bounded at high ``rife_exp`` (was ~80× body velocity at ``rife_exp = 8``, now ~5× regardless of ``rife_exp``) and the post-anchor cross-pair residual collapses to its minimum visible scale (both flanking samples now sit at IFNet timesteps very close to the shared keyframe). At ``rife_exp ≤ 4`` v9's behaviour is byte-identical to v8 up to the 0.12 → 0.119 cap rounding at ``n = 16``; at ``rife_exp ≥ 5`` body / bookend bytes change. Older manifests are silently re-baked the next time RIFE runs.
