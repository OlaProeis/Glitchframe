...
| `PIXEL_SMEAR` | `pipeline/pixel_smear.py` | `apply_pixel_smear(frame, t, clips, song_hash) → ndarray` | `docs/technical/pixel-smear-renderer.md` |
| `BLOCK_GLITCH` | `pipeline/block_glitch.py` | `apply_block_glitch(frame, t, clips, song_hash) → ndarray` | `docs/technical/block-glitch-renderer.md` |
...
| `FADE` | `pipeline/fade.py` | `fade_alpha(t, clips) → [0, 1]` (black overlay) | `docs/technical/fade-renderer.md` |
...
`SCREEN_SHAKE` → `PIXEL_SMEAR` → `BLOCK_GLITCH` → `CHROMATIC_ABERRATION` → `SCANLINE_TEAR` → `COLOR_INVERT` → `FADE`.

`FADE` runs **last** so a fade-to-black overlay covers every preceding glitch / inversion (clean reveal-from-black on intros, clean fade-to-black on outros).

Each step **short-circuits** when the corresponding clip set is inactive at time `t` or the effect is neutral (e.g. shake offset `≈ 0`, invert mix `≈ 0`, fade alpha `≈ 0`), so an empty or irrelevant timeline stays a no-op (regression guard).

## Determinism and hashing

- **Screen shake**, **pixel smear**, **block glitch**, **chromatic aberration**, and **scanline tear** use `song_hash` (and `clip.id` where applicable) so repeated renders with the same cache stay bit-stable.
- **Colour invert** and **fade** are deterministic from clip geometry and `settings` alone.

## Cross-references

- **Compositor integration (merge rules, `auto_reactivity_master`, post-pass detail):** `docs/technical/effects-timeline-compositor.md`
- **Editor HTML/JS (per-kind rows, gear panel, `window._glitchframe_effects_state`):** `docs/technical/effects-timeline-editor.md`
