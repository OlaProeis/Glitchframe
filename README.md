...
- **`Could not load library cudnn_ops_infer64_8.dll` / `Could not locate cudnn_ops_infer64_8.dll` / error `1920` (often **after** `Performing voice activity detection using Silero`):** the **VAD** step is on Silero; the failure is usually **faster-whisper / CTranslate2** loading a **cuDNN 8**-named DLL while **PyTorch 2.5+ (cu124)** wheels ship **cuDNN 9**-style names, or the DLLs are not on the process search path. **Pinokio** sets ``GLITCHFRAME_WHISPERX_VAD_METHOD=silero`` (``start.js``) for the Pyannote VAD case; the remaining fix is **use CTranslate2 4.5+** on Windows (``pyproject.toml`` / ``install.js``) and a clean **torch + torchvision + torchaudio** trio from the same CUDA index, then re-run ``pip install -e ".[all]"`` or at least ``pip install -U "ctranslate2>=4.5.0,<5"``. The app calls ``os.add_dll_directory`` for ``torch\lib`` (and common ``site-packages`` layout) before loading WhisperX on Windows. **If it still fails, try in order:** (1) Reinstall a **matching trio** in one go: `python -m pip uninstall torch torchvision torchaudio` then `python -m pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu124`, then `python -m pip install -U "ctranslate2>=4.5.0,<5"` and `python -m pip install -e ".[all]"` again. (2) `pip install nvidia-cudnn-cu12` and ensure your venv’s `Lib\site-packages\nvidia\cudnn\bin` is on `PATH` (or copy those DLLs next to the stack per [NVIDIA cuDNN 8.9/9 for CUDA 12](https://developer.nvidia.com/rdp/cudnn-archive) into the venv’s `torch\lib` / `ctranslate2` folder) — last resort. Long term, **avoid** mixing an old `torch` with a newer `torchaudio` / `whisperx`. Historical note: pinning **``ctranslate2==4.4.0``** on Windows targeted an older [whisperX#899](https://github.com/m-bain/whisperX/issues/899) case; it **fights** current WhisperX (``ctranslate2>=4.5``) and **PyTorch cu124**; do not install 4.4.x for new setups.

- **Pinokio / Windows, `cudnn_ops_infer64_8` on Align lyrics:** current ``start.js`` defaults **CPU** WhisperX (``GLITCHFRAME_WHISPERX_DEVICE``) so Align lyrics should complete without that GPU DLL path; pull latest and restart. Remove that env entry only if you want faster GPU alignment and have fixed cuDNN/ctranslate2 per the bullets above.
